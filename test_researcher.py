...
async def test_save_results():
    ...
    try:
        ...
        # Debug: Check if we have findings
        print(f"Debug: Result has {len(result.findings)} findings")
        if result.findings:
            print(f"First finding title: {result.findings[0].title}")
        
        # Save to JSON
        output_file = "research_results.json"
        result_dict = result.to_dict()
        
        print(f"\nDebug: JSON has {len(result_dict.get('findings', []))} findings")
        ...
        with open(output_file, "r") as f:
            ...
            print(f"   Verified: {len(saved_data.get('findings', []))} findings in file")
        ...
        print(f"   File size: {os.path.getsize(formatted_file)} bytes\n")
        ...
        if not result.findings:
            print("\n⚠️  WARNING: No findings were extracted!")
            print("This might be due to:")
            print("  1. JSON parsing error in _extract_findings")
            print("  2. LLM response format issues")
            print("  3. Search results not being processed correctly")
            print("\nCheck the logs above for any error messages.")
